...
base <- cps_basic |>
  filter(basicwgt > 0, lfstat != 3)

unemp_groups_e <- bind_rows(
  base |> filter(age >= 16, age <= 64) |>
...
plot_data <- unemp_groups_e |>
  select(date, ends_with("_12ma")) |>
  pivot_longer(-date, names_to = "group", values_to = "unemp_rate") |>
  mutate(group = factor(group,
    levels = c("recent_grads_12ma", "college_grads_12ma", "young_workers_12ma", "all_workers_12ma"),
...
base_epop_e <- cps_basic |>
  filter(finalwgt > 0)

epop_groups <- bind_rows(
  base_epop_e |> filter(age >= 16, age <= 64) |>
    summarize(wt_num = sum(basicwgt * replace_na(emp, 0)), wt_den = sum(basicwgt), .by = c(year, month)) |>
    mutate(group = "all_workers"),
  base_epop_e |> filter(age >= 16, age <= 24, educ == 4) |>
    summarize(wt_num = sum(finalwgt * replace_na(emp, 0)), wt_den = sum(finalwgt), .by = c(year, month)) |>
    mutate(group = "college_grads"),
  base_epop_e |> filter(age >= 22, age <= 27, educ >= 4) |>
    summarize(wt_num = sum(basicwgt * replace_na(emp, 0)), wt_den = sum(basicwgt), .by = c(year, month)) |>
    mutate(group = "recent_grads"),
  base_epop_e |> filter(age >= 22, age <= 27, educ < 4) |>
    summarize(wt_num = sum(basicwgt * replace_na(emp, 0)), wt_den = sum(basicwgt), .by = c(year, month)) |>
...
plot_data_epop <- epop_groups |>
  select(date, ends_with("_12ma")) |>
  pivot_longer(-date, names_to = "group", values_to = "epop") |>
  mutate(group = factor(group,
    levels = c("recent_grads_12ma", "college_grads_12ma", "young_workers_12ma", "all_workers_12ma"),
    labels = c("Recent college graduates (aged 22-27)", "All college graduates",
               "Young workers (aged 22-27)", "All workers")
...
base_enroll <- cps_basic |>
  filter(basicwgt > 0)

enroll_groups <- bind_rows(
  base_enroll |> filter(age >= 16, age <= 64) |>
    summarize(wt_num = sum(basicwgt * schenrl, na.rm = TRUE), wt_den = sum(basicwgt), .by = c(year, month)) |>
    mutate(group = "all_workers"),
  base_enroll |> filter(age >= 22, age <= 64, educ >= 4) |>
    summarize(wt_num = sum(basicwgt * schenrl, na.rm = TRUE), wt_den = sum(basicwgt), .by = c(year, month)) |>
    mutate(group = "college_grads"),
  base_enroll |> filter(age >= 22, age <= 27, educ >= 4) |>
    summarize(wt_num = sum(basicwgt * schenrl, na.rm = TRUE), wt_den = sum(basicwgt), .by = c(year, month)) |>
    mutate(group = "recent_grads"),
  base_enroll |> filter(age >= 22, age <= 27, educ < 4) |>
    summarize(wt_num = sum(basicwgt * schenrl, na.rm = TRUE), wt_den = sum(basicwgt), .by = c(year, month)) |>
  ...
  arrange(group, year, month) |>
  mutate(
    enroll_rate_12ma = rollsum(wt_num, k = 12, fill = NA, align = "right") /
                       rollsum(wt_den, k = 12, fill = NA, align = "right") * 100,
    .by = group
  ) |>
  select(year, month, group, enroll_rate_12ma) |>
  pivot_wider(names_from = group, values_from = enroll_rate_12ma, names_glue = "{group}_12ma") |>
  mutate(date = as.Date(paste(year, month, "01", sep = "-")))

plot_data_enroll <- enroll_groups |>
  select(date, ends_with("_12ma")) |>
  pivot_longer(-date, names_to = "group", values_to = "enroll_rate") |>
  mutate(group = factor(group,
    levels = c("recent_grads_12ma", "college_grads_12ma", "young_workers_12ma", "all_workers_12ma"),
    labels = c("Recent college graduates (aged 22-27)", "All college graduates",
               "Young workers (aged 22-27)", "All workers")
...
ggsave("images/unemp_groups_enrolled.png", width = 8, height = 4.5, dpi = 300)
